...
class SelectPercentileChi2(Selector):
    ...
    def fit(self,x:np.array,y):
        ...
        valid_index_list = np.argwhere(self._scores > scores_percentile).reshape(-1)
...
class SelectMutualInfo(Selector):
    ...
    def fit(self,x:np.array,y):
        ...
        valid_index = np.argwhere(mutual_informations > 0).reshape(-1)
...
#     # var_selectors = VarianceSelector() # theshold 0.0001
